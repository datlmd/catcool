...
class Provinces_manage extends Admin_Controller
{
    protected $errors = [];
    ...
    public function __construct()
    {
        ...
        $this->lang->load('countries_provinces_manage', $this->_site_lang);
    ...
    public function index()
    {
        ...
        list($list, $total) = $this->Province->get_all_by_filter($filter, $limit, $start_index);
    ...
    public function add()
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() !== FALSE) {
            ...
            if ($this->Province->insert($additional_data) !== FALSE) {
    ...
    public function delete($id = null)
    {
        ...
        if (isset($_POST['is_delete']) && isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            $list_delete = $this->Province->where('province_id', $ids)->get_all();
            ...
            try {
                foreach($list_delete as $value) {
                    $this->Province->delete($value['province_id']);
                }
        ...
        $list_delete = $this->Province->where('province_id', $delete_ids)->get_all();
        if (empty($list_delete)) {
            json_output(['status' => 'ng', 'msg' => lang('error_token')]);
        }
        ...
        json_output(['data' => theme_view('provinces/delete', $data, true)]);
    }
    ...
    protected function get_form($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $data_form = $this->Province->get($id);
            if (empty($data_form)) {
                set_alert(lang('error_empty'), ALERT_ERROR);
                redirect(self::MANAGE_URL);
            }
        ...
        theme_load('provinces/form', $data);
    }
    ...
    public function publish()
    {
        ...
        $item_edit = $this->Province->get($id);
        if (empty($item_edit)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }

        $item_edit['published'] = !empty($_POST['published']) ? STATUS_ON : STATUS_OFF;
        if (!$this->Province->update($item_edit, $id)) {
