...
class Manage extends Admin_Controller
{
    ...
    CONST MANAGE_ROOT       = 'products/weight_class/manage';
    CONST MANAGE_URL        = 'products/weight_class/manage';
    ...
    public function __construct()
    {
        ...
        $this->lang->load('weight_class_manage', $this->_site_lang);

        //load model manage
        $this->load->model("products/Weight_class", "Weight_class");
        $this->load->model("products/Weight_class_description", "Weight_class_description");
    ...
    public function index()
    {
        ...
        list($list, $total) = $this->Weight_class->get_all_by_filter($filter, $limit, $start_index);
        ...
        theme_load('weight_class/list', $data);
    }
    ...
    public function add()
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            ...
            $id = $this->Weight_class->insert($add_data);
            if ($id === FALSE) {
            ...
            foreach (get_list_lang() as $key => $value) {
                ...
                $add_data_description[$key]['weight_class_id'] = $id;
            }
            $this->Weight_class_description->insert($add_data_description);
    ...
    public function edit($id = null)
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            ...
            if (valid_token() === FALSE || $id != $this->input->post('weight_class_id')) {
            ...
            foreach (get_list_lang() as $key => $value) {
                ...
                $edit_data_description[$key]['weight_class_id'] = $id;

                if (!empty($this->Weight_class_description->get(['weight_class_id' => $id, 'language_id' => $key]))) {
                    $this->Weight_class_description->where('weight_class_id', $id)->update($edit_data_description[$key], 'language_id');
                } else {
                    $this->Weight_class_description->insert($edit_data_description[$key]);
                }
            }

            $edit_data['value'] = $this->input->post('value', true);
            if ($this->Weight_class->update($edit_data, $id) !== FALSE) {
    ...
    protected function get_form($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $data_form = $this->Weight_class->with_details()->get($id);
            if (empty($data_form)) {
                set_alert(lang('error_empty'), ALERT_ERROR);
                redirect(self::MANAGE_URL);
            }
        ...
        theme_load('weight_class/form', $data);
    }
    ...
    public function delete($id = null)
    {
        ...
        if (isset($_POST['is_delete']) && isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            $list_delete = $this->Weight_class->get_list_full_detail($ids);
            if (empty($list_delete)) {
                json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
            }
            try {
                foreach($list_delete as $value){
                    $this->Weight_class_description->delete($value['weight_class_id']);
                    $this->Weight_class->delete($value['weight_class_id']);
                }
        ...
        $list_delete = $this->Weight_class->get_list_full_detail($delete_ids);
        if (empty($list_delete)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }
        ...
        json_output(['data' => theme_view('weight_class/delete', $data, true)]);
    }
}
