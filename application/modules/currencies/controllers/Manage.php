...
class Manage extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->load('currencies_manage', $this->_site_lang);
    ...
    public function index()
    {
        ...
        list($list, $total) = $this->Currency->get_all_by_filter($filter, $limit, $start_index);
    ...
    public function add()
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            $add_data = [
                ...
                'symbol_left' => $this->input->post('symbol_left', true),
                'symbol_right' => $this->input->post('symbol_right', true),
                'decimal_place' => $this->input->post('decimal_place', true),
                'value' => $this->input->post('value', true),
                'published' => (isset($_POST['published'])) ? STATUS_ON : STATUS_OFF,
                'ctime' => get_date(),
            ];
            $id = $this->Currency->insert($add_data);
            if ($id === FALSE) {
    ...
    public function edit($id = null)
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            ...
            if (valid_token() === FALSE || $id != $this->input->post('currency_id')) {
    ...
    protected function get_form($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $data_form = $this->Currency->get($id);
            if (empty($data_form)) {
                set_alert(lang('error_empty'), ALERT_ERROR);
                redirect(self::MANAGE_URL);
            }
    ...
    public function delete($id = null)
    {
        ...
        if (isset($_POST['is_delete']) && isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            $list_delete = $this->Currency->where('currency_id', $ids)->get_all();
        ...
        $list_delete = $this->Currency->where('currency_id', $delete_ids)->get_all();
        if (empty($list_delete)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }
    ...
    public function publish()
    {
        ...
        $item_edit = $this->Currency->get($id);
        if (empty($item_edit)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }

        $item_edit['published'] = !empty($_POST['published']) ? STATUS_ON : STATUS_OFF;
        if (!$this->Currency->update($item_edit, $id)) {
