...
class Categories_manage extends Admin_Controller
{
    protected $errors = [];
    ...
    public function __construct()
    {
        ...
        $this->load->model("news/News_category_description", 'News_category_description');
        ...
        $this->breadcrumb->add(lang('module_article'), base_url("news/manage"));
    ...
    public function index()
    {
        ...
        list($list, $total) = $this->News_category->get_all_by_filter($filter);
    ...
    public function add()
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            ...
            $id = $this->News_category->insert($add_data);
            if ($id === FALSE) {
            ...
            $this->News_category_description->insert($add_data_description);
    ...
    public function edit($id = null)
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() === TRUE) {
            ...
            foreach (get_list_lang() as $key => $value) {
                ...
                if (!empty($this->News_category_description->get(['category_id' => $id, 'language_id' => $key]))) {
                    $this->News_category_description->where('category_id', $id)->update($edit_data_description[$key], 'language_id');
                } else {
                    $this->News_category_description->insert($edit_data_description[$key]);
                }
            }
            ...
            if ($this->News_category->update($edit_data, $id) !== FALSE) {
    ...
    public function delete($id = null)
    {
        ...
        if (isset($_POST['is_delete']) && isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            $list_delete = $this->News_category->get_list_full_detail($ids);
            ...
            try {
                ...
                foreach($list_delete as $value) {
                    $this->News_category_description->delete($value['category_id']);
                    $this->News_category->delete($value['category_id']);
        ...
        $list_delete = $this->News_category->get_list_full_detail($delete_ids);
        if (empty($list_delete)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }
    ...
    protected function get_form($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $category = $this->News_category->with_details()->get($id);
    ...
    public function publish()
    {
        ...
        $item_edit = $this->News_category->get($id);
        if (empty($item_edit)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }

        $item_edit['published'] = !empty($_POST['published']) ? STATUS_ON : STATUS_OFF;
        if (!$this->News_category->update($item_edit, $id)) {
    ...
    public function update_sort()
    {
        ...
        if (isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            if (!$this->News_category->update($data_sort, "category_id")) {
                json_output(['status' => 'ng', 'msg' => lang('error_json')]);
            }
