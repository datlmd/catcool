 ...
 * 		}
 * 	}
 *   Git: https://github.com/avenirer/CodeIgniter-MY_Model
 **/
class MY_Model extends CI_Model
{
    ...
    protected $return_as = 'array';//object
    ...
    public function _prep_before_write($data)
    {
        ...
        if($multi===FALSE)
        {
            foreach ($data_as_array as $field => $value)
            {
                ...
                {
                    show_error('MY_Model: Unknown column ('.$field.') in table: ('.$this->db_table.').');
                }
            }
        }
        else
    ...
    public function insert($data = NULL)
    {
        ...
        if($multi === FALSE)
        {
            ...
            if($this->_database->insert($this->db_table, $data))
            {
        ...
        else
        {
            ...
            foreach($data as $row)
            {
                ...
                if($this->_database->insert($this->db_table,$row))
                {
    ...
    public function update($data = NULL, $column_name_where = NULL, $escape = TRUE)
    {
        ...
        if($multi === FALSE)
        {
            ...
            if($escape)
            {
                if($this->_database->update($this->db_table, $data))
                {
                    $this->_prep_after_write();
            ...
            {
                if($this->_database->set($data, null, FALSE)->update($this->db_table))
                {
                    $this->_prep_after_write();
                    $affected = $this->_database->affected_rows();
                    $return = $this->trigger('after_update',$affected);
        ...
        else
        {
            ...
            foreach($data as $row)
            {
                ...
                if($escape)
                {
                    if($this->_database->update($this->db_table,$row))
                    {
                        $rows++;
                    }
                }
                ...
                {
                    if($this->_database->set($row, null, FALSE)->update($this->db_table))
                    {
                        $rows++;
                    }
                }
    ...
    public function where($field_or_array = NULL, $operator_or_value = NULL, $value = NULL, $with_or = FALSE, $with_not = FALSE, $custom_string = FALSE)
    {
        ...
        {
            $this->_database->{$where_or}(array($this->db_table.'.'.$this->primary_key => $field_or_array));
        }
        ...
        {
            $this->_database->{$where_or}(array($this->db_table.'.'.$field_or_array => $operator_or_value));
        }
        elseif(!isset($value) && isset($field_or_array) && isset($operator_or_value) && is_array($operator_or_value) && !is_array($field_or_array))
        {
            //echo $field_or_array;
            //exit;
            $this->_database->{$where_or.$not.'_in'}($this->db_table.'.'.$field_or_array, $operator_or_value);
        }
    ...
    public function delete($where = NULL)
    {
        if(!empty($this->before_delete) || !empty($this->before_soft_delete) || !empty($this->after_delete) || !empty($this->after_soft_delete) || ($this->soft_deletes === TRUE))
        {
            ...
            $query = $this->_database->get($this->db_table);
        ...
        if($this->soft_deletes === TRUE)
        {
            if(isset($to_update)&& count($to_update) > 0)
            {
                ...
                $affected_rows = $this->_database->update_batch($this->db_table, $to_update, $this->primary_key);
        ...
        {
            if($this->_database->delete($this->db_table))
            {
    ...
    public function restore($where = NULL)
    {
        ...
        if($affected_rows = $this->_database->update($this->db_table,array($this->_deleted_at_field=>NULL)))
        {
            $this->_prep_after_write();
            return $affected_rows;
        }
    ...
    {
        $this->_database->join($this->_relationships[$requested['request']]['foreign_table'], $this->db_table.'.'.$this->_relationships[$requested['request']]['local_key'].' = '.$this->_relationships[$requested['request']]['foreign_table'].'.'.$this->_relationships[$requested['request']]['foreign_key']);
        $the_select = '';
        ...
        else
        {
            ...
            if(!empty($this->_requested))
            {
                foreach($this->_requested as $requested)
                {
                    ...
                    {
                        $this->_database->select($this->db_table.'.'.$this->_relationships[$requested['request']]['local_key']);
                    }
                }
            }
            if(isset($where))
    ...
    public function count_rows($where = NULL)
    {
        ...
        $this->_database->from($this->db_table);
    ...
    protected function join_temporary_results($data)
    {
        foreach($this->_requested as $requested_key => $request)
        {
            ...
            else
            {
                ...
                $this->_database->join($this->db_table, $pivot_table.'.'.$pivot_local_key.' = '.$this->db_table.'.'.$local_key,'left');
    ...
    private function _has_one($request)
    {
        ...
        $this->_database->join($relation['foreign_table'], $relation['foreign_table'].'.'.$relation['foreign_key'].' = '.$this->db_table.'.'.$relation['local_key'], 'left');
        return TRUE;
    }
    /**
                ...
                if(isset($this->{$option}) && !empty($this->{$option}))
                {
                    foreach($this->{$option} as $key => $relation)
                    {
                        ...
                        if(!is_array($relation))
                        {
                            ...
                            $pivot_local_key = $this->db_table.'_'.$local_key;
                        ...
                        else
                        {
                            if($this->is_assoc($relation))
                            {
                                ...
                                {
                                    $pivot_table = $relation['pivot_table'];
                                    $pivot_local_key = (array_key_exists('pivot_local_key',$relation)) ? $relation['pivot_local_key'] : $this->db_table.'_'.$this->primary_key;
                            ...
                            else
                            {
                                ...
                                if($option=='has_many_pivot')
                                {
                                    $pivot_local_key = $this->db_table.'_'.$this->primary_key;
                                    $pivot_foreign_key = $foreign_table.'_'.$foreign_key;
                        ...
                        {
                            $tables = array($this->db_table, $foreign_table);
                            sort($tables);
    ...
    private function _where_trashed()
    {
        ...
        {
            case 'only' :
                $this->_database->where($this->db_table.'.'.$this->_deleted_at_field.' IS NOT NULL', NULL, FALSE);
                break;
            case 'without' :
                $this->_database->where($this->db_table.'.'.$this->_deleted_at_field.' IS NULL', NULL, FALSE);
                break;
            case 'with' :
                break;
        }
     ...
     * which will be tested if exists. If all went well, the table name will be saved as $this->db_table
     * @return boolean
     */
    private function _fetch_table()
    {
        ...
        {
            $this->db_table = $this->_database->dbprefix($this->_get_table_name(get_class($this)));
            if (!$this->db->table_exists($this->db_table)) {
                show_error(
                    sprintf('While trying to figure out the table name, couldn\'t find an existing table named: <strong>"%s"</strong>.<br />You can set the table name in your model by defining the protected variable <strong>$table</strong>.',$this->db_table),
                    500,
    ...
    private function _set_table_fillable_protected()
    {
        if (is_null($this->fillable)) {
            ...
            $table_fields = $this->_database->list_fields($this->db_table);
