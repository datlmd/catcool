...
class Stock_status extends MY_Model
{
    public function __construct()
    {
        parent::__construct();

        $this->db_table    = "stock_status";
        $this->primary_key = "stock_status_id";

        //khoa ngoai
        $this->has_one["detail"] = [
            "foreign_model" => "products/Stock_status_description",
            "foreign_table" => "stock_status_description",
    ...
    public function get_all_by_filter($filter = null, $limit = 0, $offset = 0, $order = null)
    {
        ...
        if (!empty($filter["id"])) {
            $filter_root[] = ["stock_status_id", (is_array($filter["id"])) ? $filter["id"] : explode(",", $filter["id"])];
        }
        ...
        $order = empty($order) ? ["stock_status_id" => "DESC"] : $order;
    ...
    public function get_list_full_detail($ids)
    {
        ...
        $result        = $this->where("stock_status_id", $ids)->with_detail($filter_detail)->get_all();
