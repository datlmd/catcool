...
class Manage extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        list($list, $total) = $this->Module->get_all_by_filter($filter, $limit, $start_index);
    ...
    public function add()
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() !== FALSE) {
            $add_data = [
                ...
                'sub_module' => $this->input->post('sub_module', true),
                'user_id'    => $this->get_user_id(),
                'published'  => (isset($_POST['published'])) ? STATUS_ON : STATUS_OFF,
                'ctime'      => get_date(),
            ];

            $id = $this->Module->insert($add_data);
            if ($id !== FALSE) {
    ...
    public function edit($id = null)
    {
        ...
        if (isset($_POST) && !empty($_POST) && $this->validate_form() !== FALSE) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($this->Module->update($edit_data, $id) !== FALSE) {
    ...
    public function delete($id = null)
    {
        ...
        if (isset($_POST['is_delete']) && isset($_POST['ids']) && !empty($_POST['ids'])) {
            ...
            $list_delete = $this->Module->where('id', $ids)->get_all();
        ...
        $list_delete = $this->Module->where('id', $delete_ids)->get_all();
        if (empty($list_delete)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }
    ...
    public function publish()
    {
        ...
        $item_edit = $this->Module->get($id);
        if (empty($item_edit)) {
            json_output(['status' => 'ng', 'msg' => lang('error_empty')]);
        }

        $item_edit['published'] = !empty($_POST['published']) ? STATUS_ON : STATUS_OFF;
        if (!$this->Module->update($item_edit, $id)) {
