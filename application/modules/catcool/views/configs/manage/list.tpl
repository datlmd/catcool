				...
				<div class="card-body">
					<div class="row">
						...
						<div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-12 mb-2 text-right">
                            ...
                            {anchor("`$manage_url`/write`$params_current`", '<i class="fas fa-save mr-1"></i>'|cat:lang('btn_write'), ['class' => 'btn btn-xs btn-space btn-secondary'])}
						</div>
					</div>
					{if !empty($list)}
						<div class="table-responsive">
							<table class="table table-striped table-hover table-bordered second">
								...
								{foreach $list as $item}
									<tr>
										...
										<td>{anchor("$manage_url/edit/`$item.id``$params_current`", $item.config_key, 'class="text-primary"')}</td>
